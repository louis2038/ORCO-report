...
#definition(name: "Generator family", id: "def:generator-family")[
  ...
  The family is *closed-complete* for $𝒮$ if every model (element of $𝒮$) can be written as a finite sum of generators:
  $
...
A generator is _complete_, but it can be produced in several orders. To model this, we attach a small automaton to each $g in cal(G)$.
...
#definition(name: "Residual automaton of a generator", id: "def:residual-automaton")[
  ...
  A *copy* of the generator $g$ is a running instance of the automaton $cal(A)_g$. When the state reaches $R = 0$, the copy is *closed*: all events have been emitted. If the state is $R > 0$, the copy is *open*: the residual $R$ records the events that remain to be emitted before the copy can close.
]
...
$g = (g - R) + R$.
The first term is already visible. The second term is the future required to _close_ this copy of the generator.
...
#definition(name: "Transition condition", id: "def:transition-condition", breakable: false)[
  ...
  + $phi$ preserves the generator and reduces the residual: if we write for every $(g, R, i) in hat(OO)_eta$, $phi(g, R, i) = (g', R', i')$, then $g' = g$ and $0 <= R' <= R$.
]

We can also say: for each generator $g in cal(G)$ and each residual $0 <= R <= g$, there exists an injection
$sigma_(g,R) : {1, dots, 𝕞_eta (g,R)} -> {1, dots, 𝕞_(eta') (g,R')}$
such that $R'_(sigma_(g,R)(i)) <= R_i$ for every $i$. The map $sigma_(g,R)$ is only required to be injective, not bijective. This reflects the fact that a transition may open new copies of a generator, but it cannot close or destroy existing ones: every copy present in $eta$ must still be present in $eta'$, possibly with a smaller residual.
...
$
This fibre may contain many trajectories. Two different trajectories can explain the same observation sequence with different distributions of open and closed copies and different residual futures.
...
$
The observation sequence is _explained_ by the theory $(cal(G), xi)$ if this set is non-empty.
...
The symbol $xi$ denotes a constraint on trajectories. It is not fixed by the definition of a dynamic contextual automaton. It is an additional modelling choice. A first possible choice, and the one that initially motivated this layer, is to bound the number of open copies of each generator.
...
#remark(name: "Magnet analogy", id: "rem:magnet-analogy")[
  ...
  The magnet does not choose the destination. It prevents the trajectory from escaping into unbounded instability. More precisely, $xi_(λ,B)$ does not select a unique trajectory $eta_1 arrow.squiggly eta_2 arrow.squiggly dots arrow.squiggly eta_k$. It filters the set of admissible trajectories by removing those that violate the stability bound at some step.
]
