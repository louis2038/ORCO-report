...
The goal is to model the experiment itself, rather than only the final probability distribution. We consider one or several laboratories equipped with measurement devices. Let $X$ denote the set of available _measurements_, let $cal(M)$ denote the set of _measurement contexts_, i.e.~the subsets of measurements that can be *performed jointly*, and let $O$ be a finite set of _possible outcomes_. An experimental run consists in choosing a context $C in cal(M)$ and observing an _outcome assignment_ $s : C arrow.r O$. Thus the primitive observational events are pairs $( C , s )$, where $C$ is the performed context and $s$ is the observed local outcome.
...
#definition[
  ...
  The *event presheaf* associated with this scenario is the functor #footnote[A functor is a mapping between categories. See @Leinster_2016[Chapter 1] for a gentle introduction.] $cal(E) : cal(P)(X)^"op" -> "Set"$
  ...
  Let $R$ be a semiring. For any finite set $Y$, we define the functor $𝒟_R : Set -> Set$ like
  $
...
In the standard probabilistic setting, one works directly with normalized distributions ($t=1$) and the grading is invisible. Here, we keep the grading explicit: each level $t$ defines a slice of the theory, and the passage from one level to the next carries empirical meaning.
...
== The discrete framework: hypergraphs and semigroups
...
A compatible model $N$ is _strongly contextual_ when no global section remains compatible with these supports:
$
  ∄ g ∈ ℰ(X)
  quad "such that" quad
  ∀ C ∈ ℳ, space g|_C ∈ "supp"(N_C).
$
...
We now record the elementary consequences of the previous definitions for the $(2,2,2)$ scenario.
...
To visualize what $ℋ_("ns")$ looks like, see @fig:local-generator and @fig:PR-generator.

#proposition[
  The set of integer models $ℋ_"ns"$ is a Hilbert basis of the semigroup $𝒮_"ns"$.
]
