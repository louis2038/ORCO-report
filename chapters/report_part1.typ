...
Several lines of work explore whether general operational principles can characterize quantum theory, for instance through generalized probabilistic theories @Barrett_2007 or axiomatic reconstructions @Hardy_2001 @Chiribella_DAriano_Perinotti_2011 @Masanes_Muller_2011. These works illustrate a difficulty: if the axioms are too weak, one obtains theories larger than quantum theory; if they are too strong, the reconstruction may simply hide the quantum structure inside the assumptions.
...
A parallel phenomenon appears when the probabilistic gap is removed. In the zero-gap formulation of $"MIP"^*$, denoted $"MIP"_0^*$, the probabilistic gap is removed: the verifier accepts with probability exactly $1$ or strictly less than $1$. One might expect that removing the continuous probabilistic layer simplifies the problem. On the contrary, Mousavi, Nezhadi, and Yuen show that the resulting class is exactly $"coRE"$ @Mousavi_Nezhadi_Yuen_2020. Related work on commuting-operator formulations yields a similar computability-theoretic behaviour @Lin_2025_MIPco_coRE. Thus even after removing the probabilistic layer, the underlying structure remains rich. This is one motivation for the present report: interesting structure can persist at the level of exact acceptance, beyond probabilities.
