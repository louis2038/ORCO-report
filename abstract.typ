...
In @chap:dynamic-condition-nonlocality, we explore several dynamic constraints for characterizing quantum correlations. The investigation tests past/future product constraints, the interruption profile $k_η$ that records the per-coordinate temporal structure of an explanation, and a constraint-programming formulation for computing admissible explanations. Each constraint reveals different aspects of the classical-quantum boundary, and the exploration shows how the dynamic framework enriches the standard contextual fraction with temporal structure.
#v(1em)
