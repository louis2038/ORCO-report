...
Finally, @chap:dynamic-condition-nonlocality formulates the target problem for a quantum dynamic theory $𝕋_q = (𝒢_q, ξ_q)$: finding generators and a constraint such that a finite counting is accepted exactly when its normalization is Hilbert-space quantum. After a short open-generator baseline, we focus on the past/future product constraint $d^- d^+ <= A$ as the main candidate, describe the experimental algorithm used to test it.
#v(1em)
#align(
  center,
